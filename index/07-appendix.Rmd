...
## Linear Models for $\Delta \: field \: position | outcome_{i^*}$
...
## Residual Plots for $\Delta \: field \: position | outcome_{i^*}$

A reminder that when sampling from $\mathcal N(\Delta \: field \: position, \sigma^{2}_{\epsilon})$, values that would lead to incoherent game stats are rejected. This fixes issues with the lack of normality for the residual distributions of $\Delta \: field \: position$ when the sampled outcome $outcome_{i^*}$ is either a conversion or a turnover on downs.  
...
gridExtra::grid.arrange(
  ggplot() +
    ...
    labs(title = "Conversions by Penalty Residuals", x = "Residuals", y = "Density", 
