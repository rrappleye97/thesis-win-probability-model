...
```{r}
running.play.length.model = readRDS(file = "final-models/running.play.length.Rda")
stopped.play.length.model = readRDS(file = "final-models/stopped.play.length.Rda")
downs.normal.model = readRDS(file = "final-models/downs.glm.normal.Rda")
downs.end.half.model = readRDS(file = "final-models/downs.boosted.end.half.Rda")
downs.end.game.model = readRDS(file = "final-models/downs.boosted.end.game.Rda")
downs.ot.model = readRDS(file = "final-models/downs.boosted.ot.Rda")
downs.n.clock.stop.model = readRDS(file = "final-models/downs.n.clock.stop.gbm.Rda")
kickoff.normal.model = readRDS(file = "final-models/kickoff.simple.lm.normal.Rda")
kickoff.end.half.model = readRDS(file = "final-models/kickoff.simple.lm.end.half.Rda")
kickoff.end.game.model = readRDS(file = "final-models/kickoff.simple.lm.4th.Rda")
kickoff.ot.model = readRDS(file = "final-models/kickoff.simple.lm.ot.Rda")
kickoff.n.plays.model = readRDS(file = "final-models/kickoff.n.plays.lm.Rda")
kickoff.n.clock.stops.model = readRDS(file = "final-models/kickoff.n.clock.stop.model.Rda")
mu.model = readRDS(file = "final-models/game.outcome.mu.Rda")
sigma.model = readRDS(file = "final-models/game.outcome.sigma.Rda")
tie.model = readRDS(file = "final-models/tie.model.Rda")
game.4th.model = readRDS(file = "final-models/game.4th.model.Rda")
game.ot.model = readRDS(file = "final-models/game.ot.model.Rda")
n.kickoff.draws = 200
n.downs.draws = 100
n.xp.draws = 100
off.td.model = readRDS(file = "final-models/td.model.Rda")
turnover.td.model = readRDS(file = "final-models/turnover.td.model.Rda")
fg.make.model = readRDS(file = "final-models/fg.make.model.Rda")
punt.td.model = readRDS(file = "final-models/punt.td.model.Rda")
fg.block.td.model = readRDS(file = "final-models/fg.block.td.model.Rda")
fg.block.model = readRDS(file = "final-models/fg.block.model.Rda")
turnover.dist = readRDS(file = "final-models/turnover.dist.Rda")
converted.dist = readRDS(file = "final-models/converted.dist.Rda")
safety.dist = readRDS(file = "final-models/safety.dist.Rda")
downs.penalty.model = readRDS(file = "final-models/penalty.gbm.Rda")
turnovers.yrds.gained.models = 
  readRDS(file = "final-models/turnovers.yrds.gained.models.Rda")
downs.turnover.yrds.gained.models = 
  readRDS(file = "final-models/downs.turnover.yrds.gained.models.Rda")
fg.yrds.gained.models = readRDS(file = "final-models/fg.yrds.gained.models.Rda")
...
punts.yrds.gained.models = readRDS(file = "final-models/punts.yrds.gained.models.Rda")
penalty.conversion.prob.model = readRDS(file = "final-models/conversion.by.penalty.prob.Rda")
n.plays.lnorm.sigma = readRDS(file = "final-models/n.plays.lnorm.sigma.Rda")
...
test.set = readRDS(file = "cleaned-data-part-4.Rda")
cols.to.keep = c("down", "TimeSecs", "yrdline100", "ydstogo", "posteam", "DefensiveTeam", "posteam_timeouts_pre", "defteam_timeouts_pre", "pos.o.dvoa", "pos.d.dvoa", "pos.st.dvoa", "def.o.dvoa", "def.d.dvoa", "def.st.dvoa", "pos.line", "pos.qb.grade", "def.qb.grade", "score.diff", "comeback.score", "ot.kickoff", "ot", "TimeSecs.adj", "TimeSecs.half.to", "TimeSecs.ot.zero", "pos.line.scaled", "pos.receive.2nd.half.kickoff", "downs.clock.will.stop", "kickoff", "kickoff.clock.will.stop", "kickoff.TimeSecs.half.to", "FieldGoalResult", "pos.win", "ot.possession.count",
  "downs.TimeSecs.half.to", "kickoff.comeback.score", "downs.comeback.score", "yrdline.group", "desc", "Win_Prob", "lock.nettleton.prob", "week")
test.set = test.set[which(test.set$year == 2017), which(colnames(test.set) %in% cols.to.keep)]
test.set = test.set %>% filter(!str_detect(test.set$desc, "END"))
test.set$pos.qb.grade[which(is.na(test.set$pos.qb.grade))] = replacement.qb.grade
test.set$def.qb.grade[which(is.na(test.set$def.qb.grade))] = replacement.qb.grade
#add very prob to this zero probability so we can evaluate loss for the nfl scrapr model
test.set$Win_Prob = ifelse(test.set$Win_Prob == 0, 0.001, test.set$Win_Prob)
#remove play by play rows where an NA play was entered for the end of half or quarter
test.set = test.set %>% filter(!is.na(test.set$Win_Prob))

#predictions = c(1:nrow(test.set))
system.time(for(i in c(37569:nrow(test.set))){
  if(i %% 500 == 0){
    print(i)
  }
  suppressWarnings((
  predictions[i] = get.prediction(data = test.set[i,], running.play.length.model, stopped.play.length.model, downs.normal.model, 
  downs.end.half.model, downs.end.game.model, downs.ot.model, downs.n.clock.stops.model, 
